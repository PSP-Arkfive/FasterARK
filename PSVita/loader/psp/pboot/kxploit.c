...
void sceNetMCopyback(uint32_t * a0, uint32_t a1, uint32_t a2, uint32_t a3);
...
int doKernelExploit(void) {
    ...
    if (test_val == 0x8F154E38){
      ...
      pspDebugScreenPrintf("Vita 3.65+");
    }
    else if (test_val == 0x8FBF003C){
      libc_clock_offset = LIBC_CLOCK_OFFSET_660; // adjust offset
      pspDebugScreenPrintf("PSP 6.60+");
    }
    else {
      pspDebugScreenPrintf("Vita 3.60");
    ...
    for (int i=0; i<10; i++) // a few iterations seem to improve stability
      dummyid = sceKernelAllocPartitionMemory(PSP_MEMORY_PARTITION_USER, dummy, PSP_SMEM_High, 0x10, NULL);

    // we can use whatever memory allocated by dummy block to inject some code, thus not needing to rely on hardcoded pointers
    jump_ptr = (u32)sceKernelGetBlockHeadAddr(dummyid) + 4;
    ...
    SceUID plantid = sceKernelAllocPartitionMemory(PSP_MEMORY_PARTITION_USER, (char *)string, PSP_SMEM_High, 0x10, NULL);
