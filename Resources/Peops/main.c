...
/*int (*origAudioInit)() = NULL;
int popsAudioInitPatched(){
    static int inited = 0;
    if (!inited) {
        sceMeAudioInitPatched(NULL, NULL);
        inited = 1;
    }
    return origAudioInit();
}*/
...
void PatchPops(SceModule* mod)
{
    ...
    /*
    // Vita 3.50 pops
    HIJACK_FUNCTION(text_addr + 0x151B4, popsAudioInitPatched, origAudioInit);
    REDIRECT_FUNCTION(text_addr + 0x15088, sceMeAudioNotifyPatched);
    HIJACK_FUNCTION(text_addr + 0x65D4, spuWriteRegisterPatched, spuWriteRegister);
    HIJACK_FUNCTION(text_addr + 0x6B48, cdrTransferSectorPatched, cdrTransferSector);
    HIJACK_FUNCTION(text_addr + 0xBF94, cdrWriteRegisterPatched, cdrWriteRegister);
    */
